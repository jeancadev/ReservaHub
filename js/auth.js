...
// Elastic rubber-band overscroll effect for the auth screen
document.addEventListener('DOMContentLoaded', () => {
    ...
    const applyTransform = (y, withTransition = false) => {
        const children = Array.from(authScreen.children);
        children.forEach(child => {
            child.style.transition = withTransition ? 'transform 0.4s cubic-bezier(0.25, 1.5, 0.5, 1)' : 'none';
            child.style.transform = y === 0 ? '' : `translateY(${y}px)`;
        });
    };

    authScreen.addEventListener('touchstart', (e) => {
        if (authScreen.scrollHeight > authScreen.clientHeight && 
            authScreen.scrollTop > 0 && 
            authScreen.scrollTop + authScreen.clientHeight < authScreen.scrollHeight) {
            return; // Middle of natural scroll
        }
    ...
    authScreen.addEventListener('touchmove', (e) => {
        ...
        const isAtTop = authScreen.scrollTop <= 0;
        const isAtBottom = Math.abs(authScreen.scrollHeight - authScreen.clientHeight - authScreen.scrollTop) <= 1;

        if ((isAtTop && deltaY > 0) || (isAtBottom && deltaY < 0)) {
            if (e.cancelable) e.preventDefault();
            const friction = 0.35;
            const stretch = deltaY * friction;
            const maxStretch = window.innerHeight * 0.15;
            currentTransY = Math.sign(stretch) * Math.min(Math.abs(stretch), maxStretch);
    ...
    authScreen.addEventListener('touchend', () => {
        if (!isDragging) return;
        isDragging = false;
        if (currentTransY !== 0) {
            currentTransY = 0;
    ...
    authScreen.addEventListener('wheel', (e) => {
        const isAtTop = authScreen.scrollTop <= 0;
        const isAtBottom = Math.abs(authScreen.scrollHeight - authScreen.clientHeight - authScreen.scrollTop) <= 1;

        if ((isAtTop && e.deltaY < 0) || (isAtBottom && e.deltaY > 0)) {
            if (e.cancelable) e.preventDefault();
            currentTransY -= (e.deltaY * 0.4);
            const maxStretch = window.innerHeight * 0.12;
            if (currentTransY > maxStretch) currentTransY = maxStretch;
            if (currentTransY < -maxStretch) currentTransY = -maxStretch;
